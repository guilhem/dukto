...
void EcWin7::init(WId wid)
{
    windowId = wid;
#ifdef Q_WS_WIN
    taskbarMessageId = RegisterWindowMessage(L"TaskbarButtonCreated");
#endif
}
...
{
#ifdef Q_WS_WIN
    taskbar->SetProgressValue(windowId, value, max);
#endif
}
...
{
#ifdef Q_WS_WIN
    taskbar->SetProgressState(windowId, (TBPFLAG)state);
#endif
}
